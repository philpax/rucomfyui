...
//!
//! This library converts ComfyUI's dynamic node definitions into statically-typed
//! Rust code, enabling type-safe usage of ComfyUI nodes.
//!
//! # Example
//!
//! ```no_run
...
/// Generate typed Rust node definitions from ComfyUI ObjectInfo.
///
/// This function takes a collection of node definitions and generates Rust code
/// with type-safe wrappers for each node.
///
/// # Arguments
///
/// * `nodes` - A slice of ComfyUI node definitions
...
/// use rucomfyui_generate_nodes::generate_nodes;
///
/// # fn example(nodes: &[rucomfyui::object_info::Object]) -> anyhow::Result<()> {
/// let output_dir = Path::new("src/nodes");
/// generate_nodes(nodes, output_dir)?;
/// # Ok(())
/// # }
/// ```
pub fn generate_nodes(nodes: &[Object], output_dir: &Path) -> Result<()> {
    generate_nodes_with_config(nodes, output_dir, GenerateConfig::default())
}

/// Generate typed Rust node definitions with custom configuration.
///
/// This function is like [`generate_nodes`] but allows specifying custom
...
pub fn generate_nodes_with_config(
    ...
    if config.scrub_array_values {
        for object in &mut nodes {
    ...
    let category_tree = rucomfyui::object_info::categorize(nodes.iter());
...
fn node_processed_inputs(node: &Object) -> Result<Vec<ProcessedInput<'_>>> {
    Ok(node
        .all_inputs()
