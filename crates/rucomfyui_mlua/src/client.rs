...
impl LuaUserData for Client {
    fn add_methods<M: LuaUserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_async_method("execute", |lua, this, graph: LuaAnyUserData| async move {
            this.check_allowed("execute", this.config.execute)?;
            let graph_ref = graph.borrow::<Graph>()?;
            let workflow = graph_ref.to_workflow();
            drop(graph_ref);

            let execution = to_lua_result(this.inner.execute(&workflow).await)?;
            let result = to_lua_result(execution.outputs().await)?;
